...
# 링크
st.markdown("- 여행 준비는 [Visit Japan](https://www.japan.travel/en/) & [Visit Korea](https://english.visitkorea.or.kr/) 참고")
...
st.image("./data/후쿠오카_시골.jpg", caption="후쿠오카 시골의 아름다움 with iPhone SE", width=500)
# 동영상
...
st.image("./data/오사카_도심2.jpg", caption="오사카의 도로 with Canon EOS", width=500)
...
st.image("./data/강릉_바다.jpg", caption="강릉 해변의 풍경 with iPhone 7", width=500)
...
st.image("./data/부산_바다2.jpg", caption="부산 도심의 풍경 with iPhone SE", width=500)
st.image("./data/부산_카페1.jpg", caption="부산의 골목길 with iPhone SE", width=500)
...
# 수식
st.write("## :gray[💸 여행 경비]")
st.latex(r'\text{총 비용} = \text{교통비} * (\text{숙박비} * \text{일수 -1} ) + \text{식비}')
...
st.markdown("### :green[📶 matplotlib 그래프]")
weather_data = pd.DataFrame({
...
# Altair 차트 생성
chart = alt.Chart(weather_long).mark_area(opacity=0.7).encode(
    x=alt.X("Month:O", title="Month"),
    y=alt.Y("Temperature:Q", title="Temperature (°C)"),
    color=alt.Color("City:N", legend=None),
    row=alt.Row("City:N", sort=["Fukuoka", "Kyoto", "Osaka", "Gangneung", "Busan"])
).properties(
    width=400,
    height=60,
    title="🌡️ 지역별 월 평균 기온 (Altair row view)"
)
